...
- LiteLLMへはCloudflare OS標準のOpenAI互換Responses API providerで接続する。
...
- `gatekeeper-keycloak`がAuthorization Code + PKCEでKeycloakと連携し、検証済みemailをlogin identityとして使用する。
- Keycloak tokenと検証済みidentityはlogin完了後に破棄し、Connector resourceとしては公開しない。
...
```bash
# Keycloak本体を起動する。
sudo docker compose --env-file .env --profile keycloak --profile inference --profile cloudflareos \
  up -d keycloak

# Cloudflare OS用OIDC clientを含むrealm設定を反映する。
...
- login画面に`Continue with Keycloak`が表示され、Keycloakの認証後にCloudflare OSへloginできる。
...
# KeycloakのOIDC discoveryとCloudflare OS clientを確認する。
...
- 公開URLがHTTP 200を返す。
- Keycloakで検証済みのemail addressがCloudflare OSのaccount identityになる。
...
| `CLOUDFLARE_OS_OIDC_CLIENT_SECRET` | stack共通開発用既定値 | Keycloak confidential clientのsecret |
| `CLOUDFLARE_OS_DISABLE_PASSWORD_AUTH` | `false` | `true`の場合はKeycloak loginだけを許可 |
| `CLOUDFLARE_OS_ADMINS` | `admin,admin@admin.com` | comma区切りのCloudflare OS管理者identity |
